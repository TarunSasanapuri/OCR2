...
def finetuning(text):
    prompt_template=PromptTemplate(
        template="For the obtained {context} make sure to finetune and do the corrections according to the scenario if it's required, otherwise leave it as usual.\nfinetuning:",
        input_variables=['context']
    )
    chain=LLMChain(llm=llm,prompt=prompt_template)
    texts = chain.run(context=text)
    return texts
...
def get_result(extracted_text):
    prompt_template = PromptTemplate(
        template="get result for the {context}\n\nanswer:",
        input_variables=["context"]
    )
    chain = LLMChain(llm=llm, prompt=prompt_template)
...
def main():
    ...
    with st.sidebar:
        with st.expander("UPLOAD IMAGE FILE"):
            ...
            if "camera_mode" not in st.session_state:
                st.session_state.camera_mode = False  # by default: Camera is off
            if st.button("Open Camera"):
                st.session_state.camera_mode = True  # for enabling camera mode
    ...
    if extract_button:
        if image is None:
            ...
            try:
                ...
                # Extract text
                text = get_gemini_response(image_data, input_prompt)
                response = finetuning(text)
    ...
    if answer_btn:
        try:
            Extract_text=extract_text('output_result')
            response=get_result(Extract_text)
        except Exception as e:
